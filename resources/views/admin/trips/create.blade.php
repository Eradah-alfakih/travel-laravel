    ...
    <form action="{{ route('trips.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="from_governorate" class="form-control" required>
                ...
                <option value="{{ $governorate->id }}"
                    {{ old('from_governorate') == $governorate->id ? 'selected' : '' }}>{{ $governorate->name }}
                </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="to_governorate" class="form-control" required>
                ...
                @foreach($governorates as $governorate)
                <option value="{{ $governorate->id }}"
                    {{ old('to_governorate') == $governorate->id ? 'selected' : '' }}>{{ $governorate->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            <label for="travel_date">Travel Date:</label>
            <input type="datetime-local" name="travel_date" id="travel_date" class="form-control"
            ...
            <div id="services">
                ...
                <div class="service row">
                    ...
                    <div class="form-group col-md-2">
                        ...
                        <input type="number" name="services[{{ $index }}][seat_service_number]"
                            class="form-control seat-service-number" value="{{ $service['seat_service_number'] }}"
                            required>
                    </div>

                    <div class="form-group col-md-4">
                        <label for="services[{{ $index }}][details]">Details:</label>
                        <textarea name="services[{{ $index }}][details]" class="form-control"
                            rows="3">{{ $service['details'] }}</textarea>
                    </div>
                ...
                <div class="service row">
                    ...
                    <div class="form-group col-md-2">
                        ...
                        <input type="number" name="services[0][seat_service_number]"
                            class="form-control seat-service-number" required>
                    </div>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response.availableBuses, function(key, value) {
                    busDropdown.append($('<option>').text(value.bus_number).attr(
                        'value', value.id).attr('data-capacity', value.capacity));
                });
            }
        });
    });
